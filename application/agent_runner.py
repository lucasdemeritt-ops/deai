...
Budget:
    --budget N sets a max spend in DAI tokens (in-memory estimate).
    Without --budget the agent runs until stopped with Ctrl+C.

    In on-chain mode, the orchestrator mints real DAI to the miner —
...
ESTIMATED_COST_PER_TASK = 10.0  # DAI tokens per task (conservative)
...
def run(args):
    ...
    api_key   = args.api_key or os.getenv("DAI_API_KEY")
    loop      = args.loop
    ...
    print(f"  Model    : {model}")
    print(f"  Budget   : {budget} DAI" if budget else "  Budget   : unlimited")
    ...
    try:
        while True:
            ...
            if budget is not None and spent >= budget:
                print(f"\n[paused] Budget exhausted ({spent:.1f} / {budget} DAI).")
            ...
            try:
                ...
                print(f"     tokens={tokens}  elapsed={elapsed:.1f}s  spent={spent:.1f} DAI")
    ...
    print(f"\nStopped. Tasks completed: {tasks_run}  Estimated spend: {spent:.1f} DAI")


def main():
    parser = argparse.ArgumentParser(
        description="DAI Persistent Agent Runner — runs a funded AI agent on the network."
    )
    ...
    parser.add_argument("--budget", type=float, default=None,
                        help="Max DAI tokens to spend before pausing (default: unlimited)")
    ...
    parser.add_argument("--api-key", default=None,
                        help="API key if the orchestrator requires one (or set DAI_API_KEY)")
